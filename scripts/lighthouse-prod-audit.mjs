...
function printHelp() {
  console.log(`
用法:
  node scripts/lighthouse-prod-audit.mjs --base https://next.momichan.com --profile both --urls-file scripts/config/lighthouse-prod-urls.json --output .lighthouse-prod
  node scripts/lighthouse-prod-audit.mjs --orchestrate --base https://next.momichan.com --output .lighthouse-prod --runs 3

参数:
  --base           目标站点基础地址，默认 https://next.momichan.com
...
function createAnalysis(summary) {
  ...
  const lines = []
  lines.push('# next.momichan.com Lighthouse 单轮分析报告')
  lines.push('')
