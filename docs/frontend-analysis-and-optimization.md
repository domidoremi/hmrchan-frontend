...
- `test:perf` resets its Lighthouse Chrome and launcher profiles before every run, supports `PERF_TEST_ONLY=<name>` for low-memory targeted reruns, and prints a preview API surface diagnostic for `/api/v1/home/featured`. This prevents stale IndexedDB, CacheStorage, service worker state, or Vite SPA HTML fallback responses from being mistaken for live API-backed LCP evidence.
...
- Worktree size is large and contains many untracked optimization files; final integration must classify all 117 current status entries before commit.
- GitNexus index was refreshed after the split modules stabilized. `useHmrExploreCatalog` is now indexed with `ExplorePage.vue` as its direct caller; final commit still requires `detect_changes(scope=all)` because the worktree remains broad.
...
- Complete a worktree classification pass before commit: production code, tests, docs, generated files, and unrelated existing dirty files must be separated.
...
- Keep GitNexus indexes current after further split-module changes so impact analysis continues to cover new module names.
...
- GitNexus index refresh passed with `npx gitnexus analyze`: 5913 nodes, 10819 edges, 251 clusters, and 300 flows indexed. Post-refresh context lookup recognizes `useHmrExploreCatalog` and its direct `ExplorePage.vue` caller.
- GitNexus `detect_changes(scope=all)` after the latest performance-script changes reported 88 changed symbols across 37 indexed changed files, critical risk due to the broad worktree. Affected flows include `loadExploreContentResource`, `setupPreloader` / `buildSessionWarmupTasks`, Google callback redirect/request chains, and the frontend-health route-state scan.
...
- Current status count: 117 entries.
- Tracked modifications: 39 files.
- Untracked files: 78 files.
- Metadata and validation files: `.gitignore`, `scripts/lib/frontend-contract-audit.js`, `scripts/frontend-health-check.ts`, `scripts/audit/frontend-patterns.ts`, `scripts/test-performance.ts`, and `scripts/refresh-public-snapshots.mjs`. The `.gitignore` change is required because repository docs are ignored by default and this analysis artifact must be tracked; the contract audit change aligns the release hook contract with split router guard modules; the frontend health change bypasses service worker navigation fallback during route scans while preserving the dedicated service worker lifecycle gate; the frontend pattern audit now treats `htmlDocument.ts` as an audited prerender image-priority boundary; the snapshot refresh script now emits the small Home prerender manifest alongside full public snapshots.
- API and service tracked changes: `src/api/hmrContent.ts`, `src/services/googleAuthService.ts`, and `src/stores/auth.ts`.
- Router, shell, and runtime tracked changes: `src/router/index.ts`, `src/layouts/HmrSiteShell.vue`, and `src/hmr/runtime/hmrRouteWarmup.ts`.
- Edge prerender tracked changes: `src/edge/htmlDocument.ts`, `src/edge/__tests__/htmlDocument.spec.ts`, `src/fallbacks/generated/publicSnapshots.ts`, and `src/styles/critical.css`.
- Component tracked changes: `src/hmr/components/HmrPostCard.vue` and `src/hmr/components/__tests__/HmrPostCard.spec.ts`.
- View tracked changes: 14 route-level Vue files under `src/views`.
- Existing tracked tests changed: `ExplorePage.spec.ts`, `SettingsPage.spec.ts`, and `useHmrBrandPet.spec.ts`.
- New documentation: 1 file, `docs/frontend-analysis-and-optimization.md`.
- New snapshot artifacts: `src/fallbacks/generated/homePrerenderManifest.ts` and `public/snapshot-media`.
- New API modules: 12 files, including content types, fallbacks, utilities, platform helpers, resources, mappers, and profile content aggregation.
- New API tests: 5 files for utilities, platforms, resources, mappers, and profile content.
- New HMR composables: 18 production files.
- New HMR composable tests: 21 files.
- New HMR runtime files: 6 files; `src/hmr/runtime/homeMedia.ts` is reviewed and included in the home prewarm media collection path.
- New HMR runtime tests: 3 files.
- New router modules: 3 production files and 3 test files.
- New store tests: 1 file, `src/stores/__tests__/auth.spec.ts`.
- New view tests: 13 files.
...
| Directory structure analysis      | verified                        | Source tree and worktree classification identify API, HMR components, composables, runtime helpers, edge prerender, router, stores, views, styles, scripts, tests, and documentation.                                                                                                                                                                                                                                                                        | Refresh classification before commit because the worktree still contains 117 changed entries.                                            |
...
| Performance optimization analysis | partial                         | Public content cache, idle route warmup, dynamic GSAP preloader loading, versioned snapshot Home prerender image preload, Home primary/full staged loading, Home hero reveal removal, media helpers, first-viewport image priority props, below-the-fold Explore deferral, compact initial catalog rendering, frontend health, clean-profile Home Lighthouse, snapshot hero tests, proxy-disabled Home Lighthouse, and preview API diagnostics are recorded. | Reduce simulated Home LCP/TBT after the discovery checklist passed; target runtime/layout cost, web fonts, and large pet sprite payload. |
...
| Existing problems                 | verified                        | Large worktree, stale GitNexus coverage for new modules, production deep regression gap, facade burden, settings naming ambiguity, and mapper evolution risk are documented.                                                                                                                                                                                                                                                                                 | Close or explicitly accept each problem before release.                                                                                  |
...
- Current memory checkpoint: available memory measured at 1227 MB after the latest proxy-disabled Home Lighthouse run. Do not start more browser-heavy, build-heavy, or multi-worker validation until memory headroom is restored.
...
- Process checkpoint: the latest `bun run test:perf` stopped its preview server after using port `64982`. A separate lightweight Vite preview probe on port `51991` was started by Codex, then stopped; the port was no longer listening afterward. High memory usage remains concentrated in existing Node/Chrome/tooling processes and unrelated local servers. These processes were inspected only; no unrelated user process was terminated.
...
- Remaining completion requirements: reduce or explicitly accept the simulated Home LCP/TBT gap after snapshot hero discovery passed, run production deep regression when the headed production checklist can be completed, rerun final full gates after the performance-script and snapshot-manifest changes, keep code intelligence current after further split-module changes, and close or explicitly accept the production-deep-regression gap before marking the goal complete.
