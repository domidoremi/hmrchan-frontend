...
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 `verification_token` 查询参数
...
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 更新后自动清除配置缓存
...
- 权限: admin
- Body: `{ "platform"?: "string", "hours"?: 0, "verification_token"?: "string" }`
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 响应: `{ "task_id": "string", "status": "dispatched" }`
...
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 `verification_token` 查询参数
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 响应: `{ "task_id": "string", "status": "dispatched", "limit": 100 }`
...
- 权限: admin
- Body: `{ "failure_ids": ["uuid-1", "uuid-2"], "verification_token"?: "string" }`
- 最多支持 100 个失败日志 ID；后端仅会重试仍保有源文件且能识别平台的失败记录
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 响应: `{ "task_id": "string", "status": "dispatched", "accepted_count": 0, "accepted_ids": [], "invalid_ids": [], "missing_ids": [], "skipped_ids": [] }`
...
- 权限: admin
- Body: `{ "role_ids": [1, 2], "verification_token"?: "string" }`
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
...
- 权限: admin
- Body: `{ "name": "string", "display_name": "string", "description"?: "string", "permissions"?: ["string"], "verification_token"?: "string" }`
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 角色名不可重复
...
- 权限: admin
- Body: `{ "display_name"?: "string", "description"?: "string", "permissions"?: [], "verification_token"?: "string" }`
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 响应: 更新后的角色对象
...
- 权限: admin
- Body: `{ "permissions": ["string"], "verification_token"?: "string" }`
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 响应: 更新后的角色对象
...
- 权限: admin
- Query: `page`, `page_size` (max 100), `status` (pending|resolved|rejected|dismissed), `target_type`
...
- 权限: admin
- Body: `{ "status": "pending|resolved|rejected|dismissed", "resolution_note"?: "string", "verification_token"?: "string" }`
- 当前设备未被信任时，必须先通过 `/api/auth/verify-identity(action=admin_operation)` 或 `/api/auth/verify-password` 获取 `verification_token`
- 推荐通过请求头 `X-Verification-Token` 传递；也支持 JSON body 中的 `verification_token`
- 未提供或提供错误 token 时返回 `403`，并带 `X-Verification-Required: true`
- 响应: 更新后的举报对象
...
- 响应（`security_events` 统计来自 `security_events` 与 `visitor_sessions` 关联数据）:
...
安全事件列表。返回匿名访客、行为异常、指纹变化、IP 变化等 `security_events` 记录；`logs` 与 `events` 为同一份数据别名，IP 地址完整显示。

- 权限: admin
- Query: `hours` (1-168, default 24), `limit` (1-500, default 100), `severity`
- 响应: `{ "logs": [{ "event_type": "timing_anomaly", "severity": "warning", "ip_address": "1.2.3.4", "visitor_session_id": 1, "details": {...}, "created_at": "datetime" }], "events": [...], "total": 0 }`
