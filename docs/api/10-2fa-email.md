...
- 已启用 2FA 时返回 `400`
- Body: `{ "password": "string", "verification_token"?: "string" }`
- 当当前设备未被信任时，必须先通过 `/api/auth/verify-password` 或 `/api/auth/verify-identity(action=update_security_settings)` 获取 `verification_token`
- 响应:
...
- 权限: required（敏感操作频率限制）
- Body: `{ "code": "string", "password": "string", "verification_token"?: "string" }`
- 当当前设备未被信任时，必须先通过 `/api/auth/verify-password` 或 `/api/auth/verify-identity(action=update_security_settings)` 获取 `verification_token`
...
- 权限: required（敏感操作频率限制）
- Body: `{ "code": "string (6位)", "verification_token"?: "string" }`
- 当当前设备未被信任时，必须先通过 `/api/auth/verify-password` 或 `/api/auth/verify-identity(action=update_security_settings)` 获取 `verification_token`
...
- 响应: `LoginResp`（同登录成功）
- 若 2FA 验证通过但当前登录仍被判定为高风险，则不会立即签发 session，而是返回 `{ "requires_risk_verification": true, "pending_token": "string", "challenge_type": "email_code", "expires_in": 300 }`
...
- 权限: required（敏感操作频率限制）
- Body: `{ "password": "string", "new_email": "string", "verification_token"?: "string" }`
- 当当前设备未被信任时，必须先通过 `/api/auth/verify-password` 或 `/api/auth/verify-identity(action=change_email)` 获取 `verification_token`
- 新邮箱已被使用返回 `400`
...
- 权限: required（敏感操作频率限制）
- Body: `{ "verification_code": "string", "verification_token"?: "string" }`
- 当当前设备未被信任时，必须先通过 `/api/auth/verify-password` 或 `/api/auth/verify-identity(action=change_email)` 获取 `verification_token`
