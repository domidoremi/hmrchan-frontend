...
修改密码。修改后当前与其他会话全部失效，需要重新登录。有敏感操作频率限制。

- 权限: required
- Body: `{ "current_password": "string", "new_password": "string", "verification_token"?: "string" }`
- 当当前设备未被信任时，必须先通过 `/api/auth/verify-password` 或 `/api/auth/verify-identity(action=change_password)` 获取 `verification_token`
