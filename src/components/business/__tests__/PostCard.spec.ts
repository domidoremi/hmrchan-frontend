...
describe('PostCard no-media semantics', () => {
  it('renders no media placeholder for a text-only post with an inflated media_count', () => {
    // Production shape: backend counts a metadata record in file_count, so a
    // text-only post may surface media_count=1 without any thumbnail or stream.
    const { host } = mountPostCard(
      createPost({
  ...
  it('renders no media placeholder when media_count is positive but the post is not media-typed', () => {
    const { host } = mountPostCard(
      createPost({
        media_count: 2,
        media_type: null,
