...
export async function encrypt(plaintext: string, password: string): Promise<string> {
  ...
  // Serialized payload layout: 16-byte salt, 12-byte IV, then AES-GCM ciphertext.
...
export function generateSecurePassword(
  ...
  // Rejection sampling avoids modulo bias when mapping bytes into the selected charset.
  const maxUnbiased = 256 - (256 % charset.length)
  let password = ''
  let offset = 0
  let randomBytes = getRandomBytes(length * 2)
