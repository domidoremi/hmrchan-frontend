...
class ImagePreloader {
  ...
  preload(url: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        logger.log(`[Preload] ✅ 图片: ${url.substring(0, 50)}...`)
        resolve()
        this.processQueue()
      }
      img.onerror = () => {
  ...
  async preloadBatch(urls: string[]) {
    ...
    logger.log(`[Preload] 📦 批量预加载 ${uniqueUrls.length} 张图片`)
  ...
  async preloadPriority(urls: string[]) {
    ...
    logger.log(`[Preload] ⭐ 优先级预加载 ${uniqueUrls.length} 张图片`)
...
class VideoPreloader {
  ...
  async preloadBatchMetadata(urls: string[]) {
    ...
    logger.log(`[Preload] 📦 批量预加载 ${uniqueUrls.length} 个视频元数据`)
