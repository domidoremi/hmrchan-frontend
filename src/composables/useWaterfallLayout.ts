...
export function useWaterfallLayout(
  ...
  /**
   * 初始化 ResizeObserver 监听卡片尺寸变化
   */
  const initResizeObserver = () => {
    if (resizeObserver) return

    resizeObserver = new ResizeObserver((entries) => {
      // 检查是否有实际的尺寸变化（避免初始化时的无用触发）
      let hasRealChange = false
      for (const entry of entries) {
        const el = entry.target as HTMLElement
        // 跳过刚添加的卡片（没有位置信息）
        if (!el.style.top || el.style.top === '0px') continue
        hasRealChange = true
        break
      }
      ...
      resizeDebounceTimer = setTimeout(() => {
        if (containerRef.value) {
          applyLayout()
          debug('Card resized, layout recalculated')
        }
  ...
  const applyLayout = () => {
    ...
    items.forEach((item) => {
      ...
      // 使用 ResizeObserver 监听卡片尺寸变化（替代单纯的图片加载监听）
      if (resizeObserver && !observedCards.has(item)) {
        resizeObserver.observe(item)
        observedCards.add(item)
      }

      // 监听卡片内的图片加载完成事件（仍保留，用于首次加载）
